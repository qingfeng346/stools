    ...
    public abstract class MovieProvider<T> : IRemoteMetadataProvider<Movie, MovieInfo> where T : HttpService {
        ...
        public async Task<MetadataResult<Movie>> GetMetadata(MovieInfo info, CancellationToken cancellationToken) {
            try {
                ...
                logger.LogError($"MovieProvider.GetMovieMetadata is error : {e}");
                return new MetadataResult<Movie>();
            }
        }
        public async Task<IEnumerable<RemoteSearchResult>> GetSearchResults(MovieInfo searchInfo, CancellationToken cancellationToken) {
            ...
            try {
                logger.LogInformation($"MovieProvider.GetSearchResults Id:{id} Info:{JsonConvert.Serialize(searchInfo)}");
